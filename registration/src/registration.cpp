...
void bloob_and_flags()
{
    ...
    Omnidrive(0, 0, -100);
    first = pcl::getTime();
    while (((pcl::getTime() - first) < 28)&& app_exit)
    {
    ...
    for (int i = 0; i < number_ball; i++)
    {
        ...
        Omnidrive(0, 0, 100);
        first = pcl::getTime();
        while (((pcl::getTime() - first) < 4)&& app_exit)
        {
            boost::this_thread::sleep(boost::posix_time::milliseconds(500));
            ROS_INFO("umadam");
        }
       ...
       // Omnidrive(0, 0, -100);
        ...
        while ((ps.x == 0 || (ps.x < 230 || ps.x > 550)) && ros::ok() && app_exit)
        {
              // cout<<ps.y<<" ps.y "<<endl;
              //        cout<<ps.x<<" ps.x "<<endl;
   ...
   //  first = pcl::getTime();
   //  while(((pcl::getTime()-first)<totaltime-3) && app_exit)
   //  {
   //      boost::this_thread::sleep(boost::posix_time::milliseconds(500));
   //  }
...
int main(int argc, char **argv)
{
    ...
    // system("canberra-gtk-play -f /home/athome/Downloads/Saghi.wav");
    ...
    // system("canberra-gtk-pause -f /home/athome/Downloads/Saghi.wav");

    // system("xmms2 clear");
    // system("xmms2 add /home/athome/Downloads/Saghi.mp3");
    // system("xmms2 play");
    // boost::this_thread::sleep(boost::posix_time::milliseconds(3000));
    ...
    */
    ...
    image_transport::Subscriber imageSubscriber;imageSubscriber = imageTransport.subscribe( "/camera/rgb/image_color", 1, rosImageCallBack);
...
void Mainlogic()
{
    ...
    */
    ...
    for (int i = 1; i < argcGlobal; ++i)
    {
        ...
        else if (strcmp(argvGlobal[i], "10") == 0)
        {
            color = "blue";
            sendToSpeech("start_music");
            bloob_and_flags();
        ...
        else if (strcmp(argvGlobal[i], "11") == 0)
        {
            DropBall();
        }
...
void executeCB_object(const  athomerobot_msgs::objectGoalConstPtr &goal, object_Server *as_object )
{
    ...
    if (  (goal->input == "Object_Start") || (goal->input == "Pose_Request") )
    {
        ...
        while (((placeobject.z == 0) || (abs(placeobject.x) > 1) || (abs(placeobject.z) > 3)) && app_exit)
        {
...
void explode_bloob()
{
    for (int i = 0 ; i< BloobFExplodeNum; ++i)
    {
        ...
        // Omnidrive(0, 0, -100);
        // first = pcl::getTime();
        // while (((pcl::getTime() - first) < 14)&& app_exit)
        // {
        //     boost::this_thread::sleep(boost::posix_time::milliseconds(500));
        //     ROS_INFO("umadam");
        // }
        ...
        while (((placeobject.z == 0) || (abs(placeobject.x) > 2) || (abs(placeobject.z) > 4)) && ros::ok() && app_exit)
        {
            placeobject = DetectColor(color, 20);
            std::cout << placeobject.x << " object_result.x " << placeobject.y << " object_result.y " << placeobject.z << " object_result.z " << std::endl;
            sleep(2);
...
string whichColorDetected()
{
    ...
    while ((placeobject.z == 0) && (placeobject.x == 0) && (placeobject.y == 0) && (ros::ok()) && app_exit)
    {
        color = "yellow";
